...
#
# Generated on 2026-03-05T10:50:42Z by kres 1dd7316.
...
COPY .golangci.yml .
ENV GOGC=50
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-pcidb/root/.cache/go-build --mount=type=cache,target=/root/.cache/golangci-lint,id=go-pcidb/root/.cache/golangci-lint,sharing=locked --mount=type=cache,target=/go/pkg,id=go-pcidb/go/pkg golangci-lint fmt --config .golangci.yml
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-pcidb/root/.cache/go-build --mount=type=cache,target=/root/.cache/golangci-lint,id=go-pcidb/root/.cache/golangci-lint,sharing=locked --mount=type=cache,target=/go/pkg,id=go-pcidb/go/pkg golangci-lint run --fix --issues-exit-code 0 --config .golangci.yml

# runs govulncheck
FROM base AS lint-govulncheck
WORKDIR /src
COPY --chmod=0755 hack/govulncheck.sh ./hack/govulncheck.sh
RUN --mount=type=cache,target=/root/.cache/go-build,id=go-pcidb/root/.cache/go-build --mount=type=cache,target=/go/pkg,id=go-pcidb/go/pkg ./hack/govulncheck.sh ./...
